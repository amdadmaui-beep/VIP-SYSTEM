...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($product_name) && !empty($unit)) {
        $stmt = $conn->prepare("SELECT Product_ID FROM products WHERE product_name = ? AND unit = ? AND Product_ID != ?");
        $stmt->bind_param("ssi", $product_name, $unit, $product_id);
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    if (empty($errors)) {
        // Update database
        $stmt = $conn->prepare("UPDATE products SET product_name = ?, form = ?, unit = ?, wholesale_price = ?, retail_price = ?, is_discontinued = ?, description = ? WHERE Product_ID = ?");
        $stmt->bind_param("sssddisi", $product_name, $form, $unit, $wholesale_price, $retail_price, $is_discontinued, $description, $product_id);
                ...
                <form method="POST" class="product-form">
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="form" name="form" class="form-input" required value="<?php echo htmlspecialchars($product['form']); ?>">
                        </div>
                        <div class="form-group">
                            <label for="unit">
                                <i class="fas fa-weight-hanging"></i> Unit *
                            </label>
                            <input type="text" id="unit" name="unit" class="form-input" required value="<?php echo htmlspecialchars($product['unit']); ?>" placeholder="e.g., kg, pcs, liters">
                        </div>
                        <div class="form-group">
